...
//          if (event.shiftKey || !event.ctrlKey || event.metaKey)  console.log('hay control');
  ...
  .directive('dynamicMapDirective', function() {
    return {
      restrict: 'EA',
      scope: {},
      link: function (scope) {
        ...
        function translateLat(lat, value){
          if(!lat){lat = 0;}
          return (lat-value)*100;
        };
        function translateLng(lng,value){
            if(!lng){lng = 0;}
        ...
        function buildShape(){
          countForShapes = 0;
          console.log("buildShape ("+shapeCount+"/"+json.features.length+") "+actualCity);
